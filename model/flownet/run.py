...
class Network(torch.nn.Module):
    def __init__(self, args_strModel='default'):
        super().__init__()
        self.args_strModel = args_strModel
        ...
        state_dict = torch.load('pretrained_weight/liteflownet-' + self.args_strModel + '.pytorch', map_location='cpu')
        self.load_state_dict({ strKey.replace('module', 'net'): tenWeight for strKey, tenWeight in state_dict.items() })    # end
...
def estimate_parallel(tenOne, tenTwo):
    global netNetwork

    intWidth = tenOne.shape[3]  # Note: in batch mode, dimensions are [B,C,H,W]
    intHeight = tenOne.shape[2]

    if netNetwork is None:
        netNetwork = Network().cuda().train(False)

    # Ensure tensors are on CUDA and are float type
    tenOne = tenOne.cuda().float()
    tenTwo = tenTwo.cuda().float()
    ...
    if intWidth != intPreprocessedWidth or intHeight != intPreprocessedHeight:
        tenOne = torch.nn.functional.interpolate(
            input=tenOne, 
            size=(intPreprocessedHeight, intPreprocessedWidth), 
            mode='bilinear', 
            align_corners=False
        )
        tenTwo = torch.nn.functional.interpolate(
            input=tenTwo, 
            size=(intPreprocessedHeight, intPreprocessedWidth), 
            mode='bilinear', 
            align_corners=False
    ...
    tenFlow = netNetwork(tenOne, tenTwo)
    ...
    # Scale flow values to account for resize
    tenFlow[:, 0, :, :] *= float(intWidth) / float(intPreprocessedWidth)
    tenFlow[:, 1, :, :] *= float(intHeight) / float(intPreprocessedHeight)
